...
class NoticeCard extends StatelessWidget {
                ...
                children: [
                  Row(
                    children: [
                      LikeActionButton(
                          ...
                          onTap: DashboardLogic.didIClick(
                                  notice.interested!, userId)
                              ? () => smileOnTapBack()
                              : () {
                                  smileOnTap();
                                ...
                                () => context.read<FetchNoticeBloc>().add(
                                      FetchNoticesFromDB(
                                        params: GetNoticeParams(
                                          url:
                                              AppUrl.noticePaginationURL(1, 12),
                                        ),
                                      ),
                                    ),
                              ),
                  ...
                  Padding(
                    ...
                    child:
                      ...
                      builder: (context, state) {
                        return AppLogoButton(
                          onTap: () => DashboardLogic.didIClick(
                                  notice.requests!, userId)
                              ? {
                                  logoOnTapBack(),
                                  BlocProvider.of<RequestResolveCubit>(context)
                                      .resolveRequest(_getParams(
                                          isPositive: false, user: user)),
                                }
                              : {
                                  logoOnTap(),
                                  BlocProvider.of<RequestResolveCubit>(context)
                                      .resolveRequest(_getParams(
                                          isPositive: true, user: user)),
                                },
                          isJoin: DashboardLogic.didIClick(
                              notice.requests!, userId),
                        );
                      },
                    ),
  ...
  NotificationParams _getParams({required bool isPositive, user}) {
    return NotificationParams(
      ...
      newNotify: NoticeLogic.createResolveMap(
        ...
        category: NotifyTypeCons.newReq,
...
const double _commentWidthMultipler = 0.75;
